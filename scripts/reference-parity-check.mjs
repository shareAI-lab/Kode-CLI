...
async function main() {
  ...
  const toolSnippet = [
    ...
    "import { z } from 'zod'",
    '',
    'let getAllTools = null',
    "for (const candidate of ['./src/tools', './packages/tools/src/index.ts', '#tools']) {",
    '  try {',
    ...
    '  } catch {}',
    '}',
    "if (!getAllTools) throw new Error('getAllTools not found')",
    '',
    'let getToolDescription = null',
    "for (const candidate of ['./src/Tool', './src/core/tools/tool', './packages/core/src/tooling/Tool.ts', '#core/tooling/Tool']) {",
    '  try {',
    '    const mod = await import(candidate)',
    ...
    '}',
    '',
    'async function getInputJsonSchema(tool) {',
    '  if (tool.inputJSONSchema) return tool.inputJSONSchema',
    '  if (isZod4Schema(tool.inputSchema)) {',
    "    return z.toJSONSchema(tool.inputSchema, { target: 'draft-07', io: 'input', unrepresentable: 'throw' })",
    '  }',
    "  const { zodToJsonSchema } = await import('zod-to-json-schema')",
    '  return zodToJsonSchema(tool.inputSchema, { name: tool.name })',
    '}',
    '',
    'const tools = getAllTools()',
    'const manifest = []',
    'for (const tool of tools) {',
    '  const schema = await getInputJsonSchema(tool)',
