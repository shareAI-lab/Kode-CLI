...
describe('goal control plane', () => {
  ...
  test('preserves a pending retryAt when the schedule definition is updated', () => {
    const { service, root } = makeService(1_000)
    const cwd = join(root, 'ws')
    ...
    // `run_now` schedules an immediate retry slot on the interval.
    const runNow = service.transitionScheduleForControlPlane({
      cwd,
      sessionId,
      scheduleId,
      expectedRevision: created!.revision,
    ...
    expect(runNow.ok).toBe(true)
    if (!runNow.ok) throw new Error('expected run_now to succeed')
    expect(runNow.goal.schedule.retryAt).toBe(1_000)

    // Rebuilding the schedule with a new cadence must not drop the pending
    // retry slot: the recovered run is consumed before the next regular run.
    const updated = service.updateScheduleForControlPlane({
      cwd,
      sessionId,
      scheduleId,
      expectedRevision: runNow.goal.revision,
      schedule: { kind: 'interval', everyMs: 120_000 },
    })
    expect(updated.ok).toBe(true)
    if (!updated.ok) throw new Error('expected update to succeed')
    if (updated.goal.schedule.kind !== 'interval') {
      throw new Error('expected the interval schedule to survive the update')
    }
    expect(updated.goal.schedule.everyMs).toBe(120_000)
    expect(updated.goal.schedule.retryAt).toBe(1_000)
  })
})
